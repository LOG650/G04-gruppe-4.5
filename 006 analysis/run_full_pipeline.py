...
def run_pipeline():
    ...
    subprocess.run(["python", "006 analysis/decode_monthly_reports.py"], check=True)
    ...
    # subprocess.run(["python", "006 analysis/prepare_timeseries.py"], check=True)
    
    # 3. Prognosetrening og prediksjon
    print("\n[2/4] Trener prognosemodeller (SARIMA/ETS/ARIMA)...")
    subprocess.run(["python", "006 analysis/demand_forecasting.py"], check=True)
    
    # 4. Optimalisering og resultatberegning
    print("\n[3/4] Beregner optimal lagerstyring...")
    subprocess.run(["python", "006 analysis/inventory_optimization.py"], check=True)
    
    # 5. Generer bestillingsanbefaling for NESTE måned
    print("\n[4/4] Genererer bestillingsanbefaling...")
    
    # Last inn de nyeste prognosene
    forecast_results = pd.read_csv("004 data/forecast_results.csv", index_col=0)
    ...
    print("\nPipeline fullført! Alle resultater og visualiseringer er oppdatert i '013 fase 3 - gjennomføring/visuals/'.")
