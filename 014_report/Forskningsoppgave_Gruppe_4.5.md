...
**Modellvalg.** Når vi har flere kandidatmodeller som alle ser ut til å oppfylle forutsetningene, brukes informasjonskriterier som Akaike Information Criterion (AIC) eller Bayesian Information Criterion (BIC). AIC favoriserer modellforklaring med en straff for kompleksitet, og er det mest brukte kriteriet i praksis fordi det balanserer treffsikkerhet og generaliserbarhet på en måte som reduserer risikoen for *overfitting*. I vår analyse bruker vi automatisert grid-søk over et utvalg parameterkombinasjoner og velger modellen med lavest AIC.
...
### 2.6 Kobling mellom oppgave, primærkilder og pensumkompendiet
...
**Arbeidsmateriale brukt for utvidelser, sensitivitet og videre arbeid:**
...
SARIMA gir den klart laveste MAE-en (137 par/mnd) og MAPE-en (17,4 %), og forbedrer den naive strategien med 15,8 prosent på MAE og 8,4 prosent på MAPE. ARIMA uten sesongledd er den klart svakeste modellen og gjør faktisk dårligere enn naiv baseline, hvilket er det formelle beviset på at sesongleddet er nødvendig: når man fjerner sesongkomponenten fra modellen, mister den evnen til å skille mellom høysesong og lavsesong. Dette resultatet er konsistent med den teoretiske forventningen i Box et al. (2015) og Hyndman og Athanasopoulos (2021), og er et entydig empirisk argument for å bruke SARIMA fremfor sesongløs ARIMA i denne typen problemstilling.
...
Vi anbefaler at Skoringen vurderer å dele **rullerende SARIMA-prognoser** med leverandøren. Det vil ikke endre bestillingsregimet (det er bundet av leverandørens egen produksjonssyklus), men det vil gi leverandøren et bedre grunnlag for sin egen kapasitetsplanlegging. Dette er konsistent med Vendor Managed Inventory (VMI)-litteraturen hos Christopher (2016) og med multi-echelon-rammeverket hos Silver et al. (2016) – som strukturelt knytter seg til den ressursbegrensede planleggingslitteraturen oppsummert av Hartmann og Briskorn (2010) – som alle formaliserer hvordan basisvarer-policyer og koordinert ressursplanlegging mellom ledd reduserer den totale varianseksponeringen i kjeden.
...
**Et pedagogisk bidrag.** Oppgaven viser hvordan etablerte temaer som ofte presenteres adskilt (SARIMA, newsvendor, bullwhip) kan kombineres til en sammenhengende beslutningsstøttemodell – et konkret eksempel på hybrid løsningsstrategi i Puchinger og Raidls (2005) forstand. Mappingtabellen i kapittel 2.6 dokumenterer 22 pensumseksjoner som er anvendt aktivt som arbeidsredskap i analysen, og kan brukes som referanse for tilsvarende bachelor- og masterprosjekter ved Høgskolen i Molde.
...
### 6.4 Videre arbeid
Primærlitteraturen og pensumkompendiet peker mot flere naturlige utvidelser av studien. Vi har gruppert dem etter modenhet og forventet effekt:

**Høyt prioritert (moderat innsats, høy forventet effekt):**
- **ARIMAX med eksogene variabler** (Box et al., 2015; Hyndman & Athanasopoulos, 2021): Inkluder værvarsel, planlagte kampanjer og lokale begivenheter for å redusere $\sigma_{\text{mnd}}$. Lv et al. (2023) viser at værinformasjon alene kan redusere prognosefeilen med 10–20 prosent i klesdetaljhandelen.
- **Bedre $\sigma$-estimering med Monte Carlo og bootstrap** (Vose, 2008; Efron & Tibshirani, 1993): erstatt punktestimat for $\sigma$ med en fordeling som propageres gjennom newsvendor-formelen via simulering eller bootstrap-resampling av residualene.
...
- **Multi-produkt $(Q,R)$ med delt kapasitet** (Silver et al., 2016): løft modellen fra aggregert sko til SKU/størrelse-nivå.
- **Flerlokasjonsmodell under usikkerhet** (Silver et al., 2016; Hartmann & Briskorn, 2010): butikk + eksternt lager + søsterbutikker som ett stokastisk, ressursbegrenset nettverk.
- **ABC/XYZ-klassifisering** (Silver et al., 2016): segmenter sortimentet før SKU-modellering, slik at den kostbare prognoseinnsatsen rettes mot A/X-modellene.
- **Multi-echelon-koordinering** (Silver et al., 2016; Hartmann & Briskorn, 2010): basisvarer-policy mellom Skoringen Råholt og leverandøren ved deling av rullerende prognose.
- **ML-prognose ved utvidet datavolum** (Puchinger & Raidl, 2005, om hybride metoder): LightGBM/gradient boosting på SKU/dag-nivå, gjerne kombinert med SARIMA som hybrid, når datamengden vokser tilstrekkelig (typisk fra 2027 og fremover).

**Lavere prioritet (større innsats eller mer spesialisert anvendelse):**
- **UFLP-evaluering av eksternt lager** (klassisk fasilitetslokaliseringsteori): kvantifiser om det eksterne lageret er økonomisk berettiget under reduserte sesongtopper.
- **Slotting-optimalisering** (Pinedo, 2016; Slack & Brandon-Jones, 2019): tilordne hyllene basert på SKU-omløp.
- **Returmodellering** (Silver et al., 2016): separat tidsserie for returer med Weibull-levetid; disposisjonsbeslutning (selge ut, returnere, donere) for sesongrest.
- **Leverandørvalg med hybride flerkriteriemetoder** (Puchinger & Raidl, 2005): formalisere vurderingen av alternative leverandører med kortere ledetid (operasjonalisert som AHP+TOPSIS eller liknende).
- **Revenue sharing-kontrakt** (Petruzzi & Dada, 1999; Silver et al., 2016): forhandle frem risikodelingsavtale der overskuddslager kan returneres mot delvis kreditt.
- **Robust optimalisering** (Vose, 2008): minimax regret-strategi som alternativ til newsvendor når SARIMA-prognosen har høy varians (f.eks. trendskift).
- **Stresstest og scenarioanalyse** (Vose, 2008): hvordan modellen reagerer på sjokk som lockdown, leverandørbrudd eller plutselig motskift.
...
| `sesongnewsvendor.py` | **Hovedanalyse:** newsvendor-bestilling per sesong |
