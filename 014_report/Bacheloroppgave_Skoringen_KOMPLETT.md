...
### 2.5 Kobling mellom oppgave og pensumkompendiet
...
Studien er en kvantitativ casestudie med deduktiv tilnærming. Vi anvender etablerte teorier fra pensum (SARIMA, newsvendor, bullwhip) på empiri fra én bedrift. Det styrker den interne validiteten at vi har full kontroll over datakvalitet og forutsetninger, men det begrenser den eksterne validiteten – funnene må valideres på flere butikker før de kan generaliseres.

### 3.2 Datafangst (FS1)
Skoringens daglige salgsrapporter foreligger som PDF-filer (én pr måned, ca. 1 100 dagsrapporter totalt). PDF er et visningsformat uten strukturert tabellinformasjon. For å kunne bruke disse i en kvantitativ analyse utviklet vi en pipeline i Python:

1. **PDF-parsing:** Vi bruker `pdfplumber` til å trekke ut tekst med koordinatinformasjon. Tabellkolonner identifiseres via x-koordinater (kolonnen "Antall par" ligger ved x ≈ 400, "Omsetning" ved x ≈ 500).
2. **Linje-validering:** Regex-mønsteret `^\d{6}` brukes for å verifisere at hver linje starter med et gyldig sekssifret varenummer. Linjer som ikke matcher (overskrifter, summer, tomme linjer) forkastes.
3. **Aggregering:** Daglige rader aggregeres til måneds- og årsnivå. Returer (negative salgsbeløp) trekkes fra netto-etterspørselen.
4. **Kvalitetskontroll:** Sum av linjer kontrolleres mot "Total salg"-feltet i bunnen av PDF-en.

Pipelinen genererer to CSV-filer som er datagrunnlaget for resten av analysen: `skoringen_salgsdata_clean.csv` (dagsdata) og `skoringen_monthly_clean.csv` (månedsdata, 36 observasjoner over 2023–2025).

### 3.3 Datavasking
- **Returer** håndteres som negative salg og trekkes fra netto-etterspørselen. Pensumets Ch09 §3 (revers-nettverk) og Ch09 §4 (Weibull-retur) viser hvordan returer i prinsippet kan modelleres som egen tidsserie med levetidsfordeling, men datavolumet vårt (≈3 års dagsdata) gir for få retur-observasjoner til at en separat Weibull-tilpasning er statistisk forsvarlig. Aggregeringen til netto-etterspørsel er derfor det metodiske valget her, mens en separat retur-modell er aktuell ved utvidelse av studien.
- **Uteliggere** identifiseres via Z-score (terskel 3) og inspiseres manuelt før eventuell korrigering.
- **Frekvenskonvertering** fra dag til måned via `pandas.resample('MS')` glatter ut daglig støy og fremhever sesongsignalet.

### 3.4 Modellering (FS2)
Tre modeller estimeres på treningssettet (2023–2024) og evalueres mot testsettet (2025):

1. **Naiv baseline:** $\hat{Y}_{t} = Y_{t-12}$ (samme måned i fjor).
2. **ETS (Holt-Winters):** additiv trend og additiv sesong, periode 12.
3. **ARIMA(1,1,1):** uten sesongledd – brukes som kontroll for å vise at sesongleddet er nødvendig.
4. **SARIMA(1,1,1)(1,1,1)$_{12}$:** med sesongledd, estimert via `statsmodels.SARIMAX` med automatisert grid-søk over kandidater, der valg av endelig modell er gjort etter laveste AIC.

En ML-baseline (LightGBM) er vurdert med utgangspunkt i pensumets Ch01 §5 (mange-variabler), men forkastet for hovedanalysen: med kun 24 treningsobservasjoner (2023–2024 på månedsnivå) blir gradient boosting-modeller sterkt overfit-utsatt sammenlignet med en parametrisk SARIMA. LightGBM er anbefalt som modell ved utvidelse til SKU/dag-nivå (kapittel 6.4), der datavolumet blir tilstrekkelig.

Evalueringsmål: MAE, RMSE og MAPE på testperioden.

### 3.5 Newsvendor-implementering (FS3)
For hver sesong $i \in \{\text{vår}, \text{høst}\}$ beregnes:

- $\mu_i$ = sum SARIMA-prognose for sesongmånedene
- $\sigma_i = \sigma_{\text{mnd}} \cdot \sqrt{n_i}$, der $\sigma_{\text{mnd}}$ er RMSE pr måned (estimert til 222,2 par fra 2025-residualene) og $n_i$ er antall måneder i sesongen
- $Q^*_i = \mu_i + z_\alpha \cdot \sigma_i$ med $z_\alpha = \Phi^{-1}(\text{CR})$

For sammenligning bruker vi en **naiv strategi** der bestillingen settes lik fjorårets faktiske sesongsalg ($Q^{\text{naiv}}_i = Y_{i,t-1}$). Dette er en realistisk approksimasjon av dagens praksis basert på samtaler med butikkeier.

På SKU-nivå (størrelser pr modell) ville den korrekte formuleringen være en multi-produkt $(Q,R)$-modell med delt kapasitet (pensumets Ch02 §3). Vi avgrenser oss til aggregert SKU i hovedanalysen, men bruker Ch02 §3 som rammeverk for utvidelsen som drøftes i §5.3 og §6.4.

### 3.6 Økonomisk evaluering (FS4)
For hver bestillingsstrategi $k$ beregner vi:

- **Bruttoresultat:** $p \cdot \min(Q_k, D) + s \cdot \max(0, Q_k - D) - w \cdot Q_k$
- **Alternativkostnad ved tapt salg:** $(p - w) \cdot \max(0, D - Q_k)$
- **Netto:** Bruttoresultat – alternativkostnad

der $D$ er faktisk sesongsalg. Antatte enhetspriser: $p = 1\,200$, $w = 600$, $s = 400$ NOK/par. Disse er **estimat** og er kontrollert for sensitivitet i kapittel 4.4.
...
Datasettet består av 36 månedsobservasjoner fra januar 2023 til desember 2025, totalt 29 619 par solgt. Årssummene er 9 041 (2023), 9 778 (2024) og 10 800 (2025), som viser en stigende trend på ca. 9 % pr år.

<div align="center">
  <img src="../013_gjennomforing/visuals/01_omsetning_over_tid.png" alt="Månedlig omsetning 2023-2025" width="80%">
  <p align="center"><small><i>Figur 4.1 Månedlig totalomsetning hos Skoringen Råholt fra januar 2023 til desember 2025.</i></small></p>
</div>

Sesongmønsteret er distinkt og stabilt på tvers av år. Tabell 4.1 viser gjennomsnittlig månedssalg over treårsperioden.

| Måned | Snitt (par) | | Måned | Snitt (par) |
|---|---:|---|---|---:|
| Januar | 510 | | Juli | 775 |
| Februar | 820 | | August | 1 024 |
| Mars | 825 | | September | 1 120 |
| April | 1 028 | | Oktober | 643 |
| Mai | 971 | | November | 750 |
| Juni | 808 | | Desember | 600 |

*Tabell 4.1 Gjennomsnittlig månedssalg 2023–2025. Variasjon høyeste/laveste måned: 2,2x.*

Vi ser at april og september er de tyngste månedene. Januar er klart svakest. Forholdet høyeste/laveste måned er 2,2x – ikke 4x som tidligere antatt – men nok til å gjøre tradisjonell EOQ uegnet og rettferdiggjøre en sesongmodell.

<div align="center">
  <img src="../013_gjennomforing/visuals/02_sesongtrender_sammenligning.png" alt="Sesongtrender på tvers av år" width="80%">
  <p align="center"><small><i>Figur 4.2 Sammenligning av sesongmønster 2023–2025. Mønsteret er stabilt på tvers av år, noe som er en gunstig egenskap for SARIMA-modellering.</i></small></p>
</div>

### 4.2 Prognosepresisjon (FS2)
Modellene ble estimert på 2023–2024 og evaluert mot 2025. Tabell 4.2 oppsummerer feilmålene.

| Modell | MAE (par) | RMSE | MAPE | Forbedring vs Naiv |
|---|---:|---:|---:|---:|
...
*Tabell 4.2 Prognosepresisjon 2025 (out-of-sample). SARIMA er beste modell på MAE og MAPE.*

Resultatene støtter at sesongleddet er nødvendig: ARIMA uten sesong er klart svakest, mens SARIMA gir lavest MAE. På årsbasis traff SARIMA-prognosen for 2025 med 10 530 par mot faktisk 10 800 – et avvik på 2,5 %.

<div align="center">
  <img src="../013_gjennomforing/visuals/demand_forecast_comparison.png" alt="Prognosesammenligning" width="80%">
  <p align="center"><small><i>Figur 4.3 SARIMA-, ARIMA- og ETS-prognose mot faktisk salg 2025. SARIMA fanger sesongtoppen i april og september bedre enn de øvrige modellene.</i></small></p>
</div>

Residualanalysen viser at SARIMA-residualene oppfyller kravene til hvit støy: Ljung-Box gir p-verdi > 0,05, og histogrammet er tilnærmet normalfordelt rundt null. Dette betyr at modellen ikke har gjenværende systematiske mønstre i feilene, og at vi kan bruke residualenes RMSE som proxy for prognoseusikkerheten $\sigma$ i newsvendor-formelen.

### 4.3 Newsvendor-bestilling (FS3)
Med antatte enhetspriser $p = 1\,200$, $w = 600$, $s = 400$ NOK/par blir det kritiske forholdet:

$$\text{CR} = \frac{p - w}{p - s} = \frac{600}{800} = 0{,}75 \implies z_\alpha = 0{,}6745$$

Dette tilsier et optimalt servicenivå på 75 %. Med $\sigma_{\text{mnd}} = 222$ par blir sesongstandardavviket $\sigma_i = 222 \cdot \sqrt{6} = 544$ par, og sikkerhetslageret $z_\alpha \cdot \sigma_i = 367$ par pr sesong.

Tabell 4.3 viser bestillingsanbefalingene for 2025-sesongen sammenlignet med naiv strategi og faktisk realisert salg.

| Sesong 2025 | $\mu_{\text{SARIMA}}$ | Sikkerhetslager | $Q^*$ | $Q_{\text{naiv}}$ | Faktisk salg |
|---|---:|---:|---:|---:|---:|
| Vår (mar–aug) | 5 663 | 367 | **6 030** | 5 389 | 6 000 |
| Høst (sep–feb) | 3 201 | 367 | **3 568** | 4 120 | 3 657 |

*Tabell 4.3 Sesongbestilling etter newsvendor vs naiv strategi for 2025.*

Det interessante mønsteret: Newsvendor anbefaler å bestille **mer** enn naiv-strategien om våren (fordi etterspørselen er stigende og fjorårssalget undervurderer behovet) og **mindre** om høsten (fordi prognosen ser at sept-toppen er konsentrert og naiv-strategi overestimerer det totale sesongbehovet).

<div align="center">
  <img src="../013_gjennomforing/visuals/newsvendor_profit_curve.png" alt="Newsvendor profittkurve vår 2025" width="80%">
  <p align="center"><small><i>Figur 4.4 Forventet bruttoresultat som funksjon av bestilt mengde for vårsesongen 2025, beregnet etter newsvendor-formelen. Maksimum nås ved Q* = 6 030 par.</i></small></p>
</div>

### 4.4 Økonomisk effekt og lagerprofil (FS4)
Realisert økonomi for de to strategiene under 2025-etterspørselen:

| | Naiv strategi | Newsvendor-strategi | Differanse |
|---|---:|---:|---:|
| Bestilling vår | 5 389 par | 6 030 par | +641 |
| Bestilling høst | 4 120 par | 3 568 par | –552 |
| Tapt salg vår | 611 par | 0 par | –611 |
| Overskuddslager høst | 463 par | 0 par | –463 |
...
Hovedgevinsten kommer fra at newsvendor unngår å undervurdere vårsesongen, noe naiv-strategien gjør fordi etterspørselen vokser fra år til år. På høsten er gevinsten mindre, men strategien sparer kapital som ellers ville vært bundet i overskuddslager.

<div align="center">
  <img src="../013_gjennomforing/visuals/inventory_newsvendor_2025.png" alt="Lagerprofil 2025" width="80%">
  <p align="center"><small><i>Figur 4.5 Simulert lagernivå gjennom 2025 ved begge strategier. Begge overstiger den fysiske kapasiteten på 3 000 par i mars – dette er en strukturell konsekvens av to-bestillingsregimet, ikke av strategivalget. Newsvendor reduserer overlageret i høstsesongen.</i></small></p>
</div>

#### 4.4.1 Sensitivitet for prisparametere
Siden enhetsprisene er estimat, gjennomførte vi en sensitivitetsanalyse av newsvendor-løsningen for variasjoner i $p$, $w$ og $s$. Tabell 4.5 viser hvordan kritisk forhold og $Q^*$ for vårsesongen endres.
...
*Tabell 4.5 Sensitivitet av $Q^*$ vår 2025 for endringer i $p, w, s$.*

Resultatet er robust: $Q^*$ varierer med under ±10 % over rimelige scenarier, og rangeringen newsvendor > naiv beholdes i alle scenarioer. Den mest sensitive parameteren er restverdien $s$ – jo lavere restverdi, jo hardere straffes overstock og jo nærmere $\mu$ trekkes $Q^*$.
...
Et tidligere narrativ i prosjektet var at "informasjon erstatter fysisk lager". For Skoringens situasjon er dette en for sterk påstand. Med to bestillinger pr år er sesongbehovet på 5 000–6 000 par et **strukturelt minstevolum** som må passere gjennom butikken. Newsvendor-modellen reduserer ikke dette volumet – den optimaliserer hvor mye av det som bestilles av riktig produkt. Pensumets Ch07 §5 (integrert lagerplanlegging) underbygger denne nyansen: informasjon er beslutningsstøtte for **hva** og **når**, mens fysisk areal er bestemt av strukturen i forsyningskjeden.

Spørsmålet om det eksterne lageret bør elimineres er i sin natur en fasilitetsbeslutning. Pensumets Ch04 §3 (UFLP) gir det formelle rammeverket: man veier åpningskost mot reduserte transportkostnader og dobbelthåndteringer. For Skoringen indikerer våre tall at en bedre prognose alene ikke fjerner behovet for ekstern kapasitet i de mest belastede vårmånedene – behovet er strukturelt. Pensumets Ch07 §3 (slotting) er likevel relevant for å utnytte intern kapasitet bedre: ved å plassere de mest etterspurte modellene (klasse A) på de mest tilgjengelige hyllene, kan man redusere intern dobbelthåndtering uavhengig av bestillingsstrategi.

Den reelle gevinsten er derfor todelt:
1. **Færre tapte salg** når SARIMA fanger en stigende trend som naiv-strategi ikke ser.
2. **Mindre overlager** når SARIMA fanger at fjorårets høst var en enkelthendelse og ikke et nytt nivå.

Begge effektene knytter seg til **prognoseusikkerheten** $\sigma$. Lavere $\sigma$ gir lavere sikkerhetslager og mer presise bestillinger. Dette er den teoretiske begrunnelsen for å investere videre i prognosearbeid.

### 5.2 Bullwhip-effekten og leverandørsamarbeid
Pensumets Ch05 §3 viser at bullwhip-forsterkningen reduseres når detaljisten deler informasjon med leverandøren. To store sesongbestillinger gir leverandøren et meget volatilt etterspørselssignal. Vi anbefaler at Skoringen vurderer å dele rullerende SARIMA-prognoser med leverandøren – ikke for å øke bestillingsfrekvensen (som ikke er mulig innenfor regimet), men for å gi leverandøren et grunnlag for sin egen kapasitetsplanlegging. Dette er i linje med VMI-litteraturen (Christopher, 2016) og pensumets Ch05 §4 (multi-echelon Clark-Scarf), som formaliserer hvordan basisvarer-policyer mellom ledd reduserer total varianseksponering. På lengre sikt kan dette åpne for forhandling om delt risiko (revenue sharing-kontrakter, Ch05 §5).

### 5.3 Begrensninger og forutsetninger
- **Aggregeringsnivå:** Modellen behandler skosalget som én SKU. I realiteten er størrelsesfordelingen kritisk – en sesongbestilling på 6 030 par er bare verdifull hvis fordelingen mellom størrelser treffer behovet. Pensumets Ch02 §4 (flerlokasjon stokastisk) gir et rammeverk for å utvide modellen til SKU-nivå.
- **Sigma-estimering:** Vi bruker SARIMA-residualene fra 2025 som proxy for fremtidig prognoseusikkerhet. Dette kan være for optimistisk hvis 2025 var et "rolig" år, eller for pessimistisk hvis modellen forbedres med mer data.
- **Prisparametere:** $p, w, s$ er estimat. En presis kalkyle krever transaksjonsdata fra Skoringens regnskap. Sensitivitetsanalysen i 4.4.1 viser at konklusjonen er robust, men de absolutte tallene kan endres med ±10–15 %.
- **Returer og bytter:** Vi modellerer netto-etterspørsel etter returer. Faktisk lagerflyt kan ha større variasjon når returer behandles separat.
- **Eksterne faktorer:** Vær, konkurrentkampanjer og makroøkonomi er ikke inkludert. Pensumets Ch01 §4 (ARIMAX) viser hvordan dette kan håndteres i en utvidet modell.

### 5.4 Bærekraft
Bedre prognoser reduserer overstock og dermed andelen sko som må selges som ukurans eller destrueres. I en bransje hvor produksjon står for en betydelig andel av avtrykket, gir dette et reelt bidrag til FNs bærekraftsmål 12 (Ansvarlig forbruk og produksjon). Pensumets Ch08 §5 (integrert grønn forsyningskjede) formaliserer denne logikken: redusert prognosefeil og redusert dobbelthåndtering oversettes direkte til lavere $\text{CO}_2$-fotavtrykk i vare- og transportstrømmen. Effekten er imidlertid avhengig av at leverandøren også reduserer sin overproduksjon basert på de delte prognosene – ellers flyttes overstocket bare ett ledd opp i kjeden, jf. bullwhip-diskusjonen i 5.2.

### 5.5 Implementering og endringsledelse
Den største praktiske barrieren er ikke teknisk, men organisatorisk. Daglig leder skal gå fra erfaringsbasert til modellbasert beslutningstaking. Vi anbefaler en gradvis innføring:

1. **Parallellkjøring (sesong 1):** Modellen genererer en anbefaling, men daglig leder fatter den endelige bestillingen. Avvik dokumenteres.
2. **Hybridkjøring (sesong 2–3):** Modellen brukes som primær input, men daglig leder kan justere ±10 % basert på lokal kunnskap (planlagte kampanjer, byggearbeid utenfor butikken etc.).
3. **Modellkjøring (sesong 4 og videre):** Modellen brukes som hovedinput, med human-in-the-loop kun for unike hendelser.

Innenfor to-bestillingsregimet er det også verdt å vurdere alternativ leverandørstruktur. Pensumets Ch10 §3 (AHP+TOPSIS for leverandørvalg) gir et flerkriterierammeverk der pris, ledetid, fleksibilitet og kvalitet vektes mot hverandre. Pensumets Ch03 §5 (MRP-lotstørrelse) viser tilsvarende at lotstørrelse ikke trenger å være statisk – den kan tilpasses sesongvis basert på prognosen, så lenge leverandøren aksepterer variabel ordrestørrelse innenfor de to årlige vinduene.
...
### 6.1 Hovedfunn
Studien har vist at SARIMA-baserte etterspørselsprognoser kombinert med newsvendor-modellen gir en estimert årlig forbedring i nettoresultat på 713 621 NOK (+14,4 %) for Skoringen Råholt under 2025-data og antatte enhetspriser. SARIMA-modellen reduserer prognosefeilen med 15,8 % mot en naiv "samme måned i fjor"-baseline (MAE 137 vs 163 par/mnd). Newsvendor-modellen anbefaler å bestille 6 030 par på vårsesongen og 3 568 par på høstsesongen i 2025, mot henholdsvis 5 389 og 4 120 par for naiv strategi.

### 6.2 Svar på forskningsspørsmålene
- **FS1:** PDF-pipelinen ekstraherer dagsdata fra ~1 100 rapporter og leverer en strukturert tidsserie med 36 månedsobservasjoner. Pipeline er reproduserbar og automatisert.
- **FS2:** SARIMA gir lavest MAE (137 par) og MAPE (17,4 %), klart bedre enn ARIMA uten sesongledd og bedre enn ETS og naiv baseline.
- **FS3:** Optimal sesongbestilling under antakelsene er $Q^*_{\text{vår}} = 6\,030$ og $Q^*_{\text{høst}} = 3\,568$ par for 2025, med sikkerhetslager 367 par pr sesong.
- **FS4:** Estimert årlig effekt er +713 621 NOK i nettoresultat, fordelt på +400 122 NOK i bruttoresultat og 313 499 NOK i redusert tapt salg.

### 6.3 Anbefalinger til Skoringen Råholt
1. **Implementer prognosebasert sesongbestilling.** SARIMA-pipelinen kjøres månedlig for å oppdatere prognosen før bestillingsfristen.
2. **Sett servicenivå eksplisitt.** Ved å beregne det kritiske forholdet kan butikken ta et bevisst valg om hvor høyt servicenivå de ønsker, snarere enn å la dette være implisitt.
3. **Del prognosene med leverandøren.** Selv uten endring i bestillingsfrekvens kan dette forbedre vilkår over tid.
4. **Loggfør realisert salg vs prognose.** Avvikene er det viktigste datagrunnlaget for å kunne forbedre modellen i fremtidige sesonger.

### 6.4 Videre arbeid
Pensumkompendiet peker mot flere naturlige utvidelser av studien:

- **ARIMAX med eksogene variabler** (Ch01 §4): inkluder værvarsel, kampanjer og lokale begivenheter for å redusere $\sigma_{\text{mnd}}$.
- **ML-prognose ved utvidet datavolum** (Ch01 §5): LightGBM/gradient boosting på SKU/dag-nivå når datamengden vokser tilstrekkelig.
...
- **Monte Carlo-sensitivitet** (Ch11 §3): kvantifisere usikkerheten i $Q^*$ ved å trekke parametere fra fordelinger snarere enn å bruke punktestimater.
...
LOG650-kompendiet, *Kvantitative metoder i logistikk* (Høgskolen i Molde, 2026). Lokal sti: `003_referanser/Kompendium/`. Innholdsfortegnelse: `003_referanser/Kompendium/00_INDEX.md`. Av kompendiets 33 seksjoner anvendes følgende 22 aktivt – jf. mappingtabellen i §2.5:
...
|---|---|---|
| $Y_t$ | Salg i måned $t$ | par/mnd |
| $\mu_i$ | Forventet sesongetterspørsel for sesong $i$ | par |
| $\sigma_i$ | Standardavvik for sesongetterspørsel | par |
| $\sigma_{\text{mnd}}$ | Prognoseusikkerhet pr måned (RMSE) | par |
| $z_\alpha$ | Standard-normal-kvantil for servicenivå $\alpha$ | – |
| CR | Kritisk forhold $(p-w)/(p-s)$ | – |
| $p$ | Utsalgspris | NOK/par |
| $w$ | Innkjøpspris fra leverandør | NOK/par |
| $s$ | Restverdi etter sesong | NOK/par |
| $Q^*$ | Optimal sesongbestilling | par |

### Vedlegg B – Datasett og kode
- Datasett: `004_data/skoringen_monthly_clean.csv`, 36 månedsobservasjoner 2023–2025.
- Pipeline: `006_analysis/run_full_pipeline.py` (PDF → CSV → SARIMA → newsvendor).
- Newsvendor-simulering: `006_analysis/sesongnewsvendor.py`.
- Resultater: `013_gjennomforing/newsvendor_resultater.json`.
- Visualiseringer: `013_gjennomforing/visuals/`.
